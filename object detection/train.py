...
def parse_args():
    ...
    # dataset
    parser.add_argument('-root', '--data_root', default='/home/huyangfan/',
                        help='dataset root')
...
def train():
    ...
    ######################AA###########################
    # dummy = torch.zeros(2*1024*1024*1024,dtype=torch.float32, device='cuda:' + str(args.local_rank))
    # del dummy
    #################################################
    ...
    if args.bit == 32:
        ...
        path_to_save = os.path.join(args.save_folder, args.dataset, args.version + '_' + str(args.bit) + 'bit')
    ...
    if args.init:
        print('Initialize from high-precision models...')
        net.load_state_dict(torch.load(args.init, map_location='cpu'), strict=False)
