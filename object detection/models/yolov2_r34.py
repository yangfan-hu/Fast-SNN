...
import numpy as np
import tools
from backbone.myresnet import MYResNet, MYSResNet


class YOLOv2R34(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20, trainable=False, conf_thresh=0.001, nms_thresh=0.6, anchor_size=None, bit=32, spike=False):
        super(YOLOv2R34, self).__init__()
        self.device = device
        ...
        # backbone
        if spike:
            self.backbone = MYSResNet(arch='resnet34',bit=bit, num_anchors = self.num_anchors, num_classes = self.num_classes, pretrain=False)
        else:
            self.backbone = MYResNet(arch='resnet34',bit=bit, num_anchors = self.num_anchors, num_classes = self.num_classes, pretrain=False)
    ...
    def init_bias(self):               
        ...
        nn.init.constant_(self.backbone.pred.block.bias[..., :self.num_anchors], bias_value)
