...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.part1 = Oneway(conv3x3(inplanes, planes, stride, bit=bit),
                            norm_layer(planes),
                            QuantReLU(inplace=True, bit=bit)) 
        
        self.part2 = Twoways(conv3x3(planes, planes, bit=bit),
                             norm_layer(planes),
...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, bit=32):
        ...
        self.groups = 1
        self.base_width = 64
        self.layer0 = Dummy(nn.Sequential(first_conv(3, 16, kernel_size=3, stride=1, padding=1, bias=False),
                                          nn.BatchNorm2d(16),
                                          QuantReLU(inplace=True, bit=bit)))
        ...
        self.layer4 = Dummy(nn.Sequential(nn.AvgPool2d(8, stride=1),
                                          nn.Flatten(1),
                                          last_fc(64 * block.expansion, num_classes)))
    ...
    def show_params(self):
        for m in self.modules():
            if isinstance(m, (QuantConv2d, QuantReLU)):
                m.show_params()
...
class Spiking_BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.part1 = Spiking_Oneway(conv3x3(inplanes, planes, stride, bit=bit),
                                    norm_layer(planes),
                                    IF(), self.T) 
        
        self.part2 = Spiking_Twoways(conv3x3(planes, planes, bit=bit),
                                     norm_layer(planes),
...
class S_ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, bit=4):
        ...
        self.layer0 = Spiking(nn.Sequential(first_conv(3, 16, kernel_size=3, stride=1, padding=1, bias=False),
                                          nn.BatchNorm2d(16),
                                          IF()), self.T)
        self.layer1 = self._make_layer(block, 16, layers[0])
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2)
        ...
        self.layer4 = last_Spiking(nn.Sequential(nn.AvgPool2d(8, stride=1),
                                                 nn.Flatten(1),
                                                 last_fc(64 * block.expansion, num_classes)), self.T)
...
def resnet20(spike=False, **kwargs):
    if spike:
        return S_ResNet_Cifar(Spiking_BasicBlock, [3, 3, 3], **kwargs)
    else:
...
def resnet32(spike=False, **kwargs):
    if spike:
        return S_ResNet_Cifar(Spiking_BasicBlock, [5, 5, 5], **kwargs)
    else:
...
def resnet44(spike=False, **kwargs):
    if spike:
        return S_ResNet_Cifar(Spiking_BasicBlock, [7, 7, 7], **kwargs)
    else:
...
def resnet56(spike=False, **kwargs):
    if spike:
        return S_ResNet_Cifar(Spiking_BasicBlock, [9, 9, 9], **kwargs)
    else:
...
def resnet110(spike=False, **kwargs):
    if spike:
        return S_ResNet_Cifar(Spiking_BasicBlock, [18, 18, 18], **kwargs)
    else:
