...
class QuantReLU(nn.ReLU):
    def __init__(self, inplace: bool = False, bit=5, power=False, additive=True, grad_scale=None):
...
#QuantAvgPool2d defauts to uniform activation quantization
class QuantAvgPool2d(nn.AvgPool2d):
    def __init__(self, kernel_size, stride, bit=5, power=False, additive=True, grad_scale=None):
        super(QuantAvgPool2d, self).__init__(kernel_size, stride)
        self.layer_type = 'QuantAvgPool2d'
        self.bit = bit
        self.power = power
        self.grad_scale = grad_scale
        if self.bit < 32:
